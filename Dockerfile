FROM asupc/qqbot
ARG NGROK_TOKEN
RUN apt-get update && apt-get install -y curl git wget unzip && git clone https://github.com/asupc/qqbot-multi-platform.git /app \
...
RUN mkdir /run/sshd \
    ...
    && echo 'dotnet QQBot.Web.dll' >> /openssh.sh \
